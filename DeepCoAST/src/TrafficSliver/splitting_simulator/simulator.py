...
parser.add_argument("-m", "--circuits", type=int, help="In how many paths is the traffic divided", default=2) 
...
parser.add_argument("-o", "--outfolder", type=str, help="Folder for output files", default='/scratch/TrafficSliver/singlesite/path2/webmd/ts') 
parser.add_argument("-w", "--weights", type=str, help="Weights for circuit (comma separated)", default='0.5,0.5') 
...
parser.add_argument("-p",'--path', nargs='+', help=' fiPath of folder with instancesles (wang_format)', default='/scratch/TrafficSliver/singlesite/original/mon_webmd')
parser.add_argument("-a", "--alpha", type=str, help="alpha values for the Dirichlet function default np.ones(m)", default='1,1')
...
def sim_bwr(n,latencies,traces,outfiles,range_, alphas):
    ...
    for instance_file in traces_file:
        ...
        if(int(instance_file.split('/')[-1].split('-')[0])%5==0): print(instance_file)
...
if __name__ == '__main__':
    ...
    if (scheme_ == 'round_robin'):
        print(paths_, latencies_, traces_, outfolder_, cells_per_circuit_)
